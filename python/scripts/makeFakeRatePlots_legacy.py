...
preSigDxyHist   = ROOT.TH1D("preSigDxyHist",   "preSigTracks' transverse distance from mumuVtx;#Deltaxy [cm];Counts", 50, 0, 0.1)
preSigDzHist    = ROOT.TH1D("preSigDzHist",    "preSigTracks' longitudinal distance from mumuVtx;#Deltaz [cm];Counts", 50, 0, 0.3)
...
preSigD0Hist    = ROOT.TH1D("preSigD0Hist",    "preSigTracks' transverse distance from beamspot;d0;Counts", 20, 0, 0.5)
...
preBkgDzHist    = ROOT.TH1D("preBkgDzHist",    "HLT tracks before filter longitudinal distance from #mu#muVtx;d_z [cm];Counts (norm.)", 50, 0, 0.3)
...
preBkgDphiHist  = ROOT.TH1D("preBkgDphiHist",  "HLT tracks before filter #phi distance from closest #mu cand;#Delta#phi;Counts (norm.)", 30, 0, 1.5)
